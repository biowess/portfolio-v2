...
export function AnimatedBackground() {
  ...
  // Star shadows – computed once, never changes
  const stars = useMemo(() => ({
  ...
  // ─── Mouse tracking ─────────────────────────────────────────────
  const rawX = useSpring(0, { stiffness: 40, damping: 18, mass: 1 });
  const rawY = useSpring(0, { stiffness: 40, damping: 18, mass: 1 });
  ...
  // Dark-variant parallax layers
  const px1 = useTransform(rawX, v => v * 14);
  const py1 = useTransform(rawY, v => v * 14);
  const px2 = useTransform(rawX, v => v * 28);
  const py2 = useTransform(rawY, v => v * 28);
  const px3 = useTransform(rawX, v => v * 50);
  const py3 = useTransform(rawY, v => v * 50);
  const nx  = useTransform(rawX, v => v * -18);
  const ny  = useTransform(rawY, v => v * -18);

  // Light-variant spotlight offset (cursor-follower)
  const lx = useTransform(rawX, v => v * 120);
  const ly = useTransform(rawY, v => v * 80);
      ...
      <AnimatePresence mode="popLayout">
            ...
            <motion.div style={{ x: nx, y: ny }} className="absolute inset-0">
              <motion.div
                ...
                style={{
                  ...
                  borderRadius: '50%',
                  background: 'radial-gradient(circle, rgba(180,20,20,0.12) 0%, rgba(140,10,10,0.06) 35%, transparent 65%)',
                  filter: 'blur(72px)',
                }}
                animate={{
                  x: [0, -100],
              ...
              <motion.div
                ...
                style={{
                  ...
                  borderRadius: '50%',
                  background: 'radial-gradient(circle, rgba(160,15,15,0.08) 0%, transparent 60%)',
                  filter: 'blur(90px)',
                }}
                animate={{
                  x: [0, -80],
        ...
        {!isDark && (
          <motion.div
            ...
            className="absolute inset-0"
          >
            {/* ── 1. Paper texture: all three effects chained in one filter ── */}
            {/*
                FIX: Previously three separate <rect> elements each had their own filter
                attribute. SVG only applies one filter per element, and the grain rects
                used fill="transparent" which gave feBlend nothing to work with (multiply
                on transparent = transparent). Now all effects are chained inside a single
                filter using result→in references, applied to one rect with a real fill.
            */}
            <svg
            ...
            >
              <defs>
                <filter id="paper-all" x="0" y="0" width="100%" height="100%">
                  ...
                  <feBlend in="withCoarse" in2="fineMuted" mode="multiply" />

                </filter>
              </defs>

              {/* Single rect with a real fill — the SourceGraphic all stages need */}
            ...
            <div
              ...
              style={{
                background:
                  'radial-gradient(ellipse at center, transparent 50%, rgba(180,160,130,0.12) 100%)',
                mixBlendMode: 'multiply',
              }}
            />

            {/* ── 3. Cursor-following spotlight (two layers) ── */}
            <motion.div
              className="absolute inset-0 pointer-events-none"
              style={{
                x: lx,
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  borderRadius: '50%',
                  background:
                    'radial-gradient(circle, rgba(255,252,235,0.6) 0%, rgba(255,248,220,0.3) 30%, rgba(255,240,210,0.08) 55%, transparent 75%)',
                  filter: 'blur(60px)',
                  transform: 'translate(-10%, -12%)',
                }}
              />
              {/* Tighter, brighter inner core */}
              <div
                className="absolute"
                style={{
                  left: '50%',
                  ...
                  borderRadius: '50%',
                  background:
                    'radial-gradient(circle, rgba(255,255,250,0.7) 0%, rgba(255,250,235,0.35) 40%, transparent 65%)',
                  filter: 'blur(28px)',
